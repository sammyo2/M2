...
public class TravelConfirmationScreen {
    ...
    public TravelConfirmationScreen(Game g, int index) {
        ...
        confirmButton.addActionListener(e -> {
            javax.swing.SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    ...
                    if ((r < 5 + g.getPlayer().getKarma()) && (distance > 0)
                            && (g.getPlayer().getShip().getCurrentFuel() != 0)) {
                        Trader t = new Trader(g);
                        TraderScreen tS = new TraderScreen(g, t, index);
                        frame.dispose();
                    } else if ((r < (5 + (5 * chance - g.getPlayer().getKarma())))
                            && (distance > 0) && (g.getPlayer().getShip().getCurrentFuel() != 0)) {
                        BanditScreen bS = new BanditScreen(g, index);
                        frame.dispose();
                    } else if ((r < (5 + (2 * (5 * chance - g.getPlayer().getKarma()))))
                            && (distance > 0)
                            && (g.getPlayer().getShip().getcurrentCargoSpace() > 0)
                            && (g.getPlayer().getShip().getCurrentFuel() != 0)) {
                        Police p = new Police(g);
                        PoliceScreen pS = new PoliceScreen(g, p, index);
                        frame.dispose();
                    } else {
            ...
            javax.swing.SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    RegionScreen rS = new RegionScreen(g,
                            g.getPlayer().getCurrentRegionIndex());
                    frame.dispose();
                }
            });
        });
