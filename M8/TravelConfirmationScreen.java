        ...
        confirmButton.addActionListener(e -> {
            javax.swing.SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    ...
                    if ((r < 5 + g.getPlayer().getKarma()) && (distance > 0) && (g.getPlayer().getShip().
                            getCurrentFuel() != 0)) {
                        Trader t = new Trader(g);
                        TraderScreen tS = new TraderScreen(g, t, index);
                        frame.dispose();
                    } else if ((r < (5 + (5 * chance - g.getPlayer().getKarma()))) && (distance > 0) && (g.
                            getPlayer().getShip().getCurrentFuel() != 0)) {
                        BanditScreen bS = new BanditScreen(g, index);
                        frame.dispose();
                    } else if ((r < (5 + (2 * (5 * chance - g.getPlayer().getKarma())))) && (distance > 0)
